...
class Patient(object):
    ...
    def update(self):
        ...
        density = len(survivor_viruses) / self.maxPop
        
        new_viruses = []
        for virus in survivor_viruses:
            try:
                new_viruses.append(virus.reproduce(density))
            except NoChildException:
                continue
            
        self.viruses = new_viruses + survivor_viruses
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    pylab.plot(avPopSize, label = "SimpleVirus")
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if all(self.isResistantTo(drug) for drug in activeDrugs):
            if random.random() <= self.maxBirthProb * (1 - popDensity):
                new_resistances = {}
                for comp in self.resistances.keys():
                    if random.random() <= self.mutProb:
                        new_resistances[comp] = not self.resistances[comp]
                    else:
                        new_resistances[comp] = self.resistances[comp]
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        """
        return [all(virus.isResistantTo(drug) for drug in drugResist) for virus in self.viruses].count(True)
    ...
    def update(self):
        ...
        for virus in survivor_viruses:
            try:
                new_viruses.append(virus.reproduce(density, self.drugs))
            except NoChildException:
                continue
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    pylab.xlabel("Time Steps")
    pylab.ylabel("Average ResistantVirus Population")
    pylab.legend(loc = "best")
    pylab.xlim(-10, timeSteps + 10)
    pylab.ylim(-10, 1000//2 + 10)
    pylab.show()'''
    
simulationWithDrug(100, 1000, 0.2, 0.05, {"pentagonol": False}, 0.00, 10, {"pentagonol": 100})
